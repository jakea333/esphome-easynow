...
namespace esphome
{
  namespace ripnetuk_ui
  {
    ...
    public:
      ...
      RipnetUkNumberComponent(const std::string &name, number::NumberMode mode, float min_value, float max_value, float step, std::function<void(float state)> on_change = nullptr);
      void setup() override;
